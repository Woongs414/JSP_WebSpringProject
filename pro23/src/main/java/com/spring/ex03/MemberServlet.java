 ...
 *//**
	 ...
	 */
/*
 * @WebServlet("/mem3.do") public class MemberServlet extends HttpServlet {
 ...
 *//**
	 ...
	 *//*
		 ...
		 * request.setCharacterEncoding("UTF-8"); MemberDAO dao = new MemberDAO();
		 * MemberVO memberVO = new MemberVO(); String action =
		 * request.getParameter("action"); String nextPage="";
		 * 
		 * if(action==null || action.equals("listMembers")) { List<MemberVO> membersList
		 * = dao.selectAllMemberList(); request.setAttribute("membersList",
		 ...
		 * }else if(action.equals("selectMemberByPwd")) { int pwd =
		 * Integer.parseInt(request.getParameter("value")); List<MemberVO> membersList =
		 * dao.selectMemberByPwd(pwd); request.setAttribute("membersList", membersList);
		 * nextPage="test02/listMembers.jsp"; } RequestDispatcher dispatch =
